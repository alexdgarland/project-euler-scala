...
class SolutionWrapperSpec extends FlatSpec with Matchers with MockitoSugar {
  ...
  behavior of "SolutionWrapper"

  it should  "have expected toString" in {
    testWrapper.toString shouldBe "1 - Solve a problem\n2 - Solve another problem"
  }

  it should "run full list when runAll is called" in {
    val wrapper = testWrapper
    wrapper.runAll()
    verify(wrapper.runner).run(mockSolutionList)
  }

  it should "not run anything when runAll is called against empty list" in {
    val wrapper = testWrapper
    wrapper.runAll()
    verify(testWrapper.runner, never()).run(_)
  }

  it should "run selected solution when runForQuestion is called" in {
    val wrapper = testWrapper
    wrapper.runForQuestion(1)
    verify(wrapper.runner).run(List(mockSolution1))
  }

  it should "throw IllegalArgumentException when runForQuestion called but solution not available" in {
