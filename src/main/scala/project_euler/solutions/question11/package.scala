...
package object question11 {
  ...
  private def maxRowIndex(grid : List[List[Int]]) = grid.length - 1

  private def maxColumnIndex(grid : List[List[Int]]) = grid.head.length - 1

  def horizontalLinesOfSize(lineSize : Int, grid : List[List[Int]]) : IndexedSeq[IndexedSeq[Int]] = {
    (0 to maxRowIndex(grid))
      .flatMap(rowIndex =>
        (0 to maxColumnIndex(grid) - (lineSize - 1))
          .map(startColumnIndex =>
            // If we don't want to genericise these functions,
            // this specific section could actually just be done as a slice of the inner lists
            (0 until  lineSize)
              .map(lineOffset =>
                grid(rowIndex)(startColumnIndex + lineOffset)
              )
          )
      )
  }

  def verticalLinesOfSize(lineSize : Int, grid : List[List[Int]]) : IndexedSeq[IndexedSeq[Int]] = {
    (0 to maxColumnIndex(grid))
      .flatMap(columnIndex =>
        (0 to maxRowIndex(grid) - (lineSize - 1))
          .map(startRowIndex =>
            (0 until  lineSize)
              .map(lineOffset =>
                grid(startRowIndex + lineOffset)(columnIndex)
              )
          )
      )
  }

  def forwardDiagonalLinesOfSize(n : Int, grid : List[List[Int]]) : List[List[Int]] = {
    throw new NotImplementedError()
    Nil
  }

  def backwardDiagonalLinesOfSize(n : Int, grid : List[List[Int]]) : List[List[Int]] = {
    throw new NotImplementedError()
    Nil
  }
